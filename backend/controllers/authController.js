...
class AuthController {
  ...
  async login(req, res) {
    try {
      ...
      if (user.password) {
        ...
        return errorResponse(res, 'Tài khoản này được đăng nhập bằng Google. Vui lòng sử dụng Google để đăng nhập.', 401);
      }

      // Kiểm tra email đã được xác nhận chưa (nếu không phải Google account)
      if (!user.google_id && !user.email_verified) {
        return errorResponse(res, 'Vui lòng xác nhận email trước khi đăng nhập. Kiểm tra hộp thư của bạn.', 403);
      }
  ...
  async resendVerificationEmail(req, res) {
    try {
      ...
      if (!user) {
        ...
        return successResponse(res, null, 'Nếu email tồn tại, email xác nhận đã được gửi');
      }
  ...
  async forgotPassword(req, res) {
    try {
      ...
      if (!user) {
        // Không tiết lộ email có tồn tại hay không
        return successResponse(res, null, 'Nếu email tồn tại, link đặt lại mật khẩu đã được gửi');
      }
  ...
  async googleCallback(req, res) {
    try {
      ...
      if (googleUser.needsPasswordSetup) {
        return res.redirect(`${process.env.FRONTEND_URL || 'http://localhost:3000'}/google-registration-success?email=${encodeURIComponent(googleUser.email)}`);
      }
  ...
  async setupPasswordForGoogle(req, res) {
    try {
      ...
      if (emailExists) {
        return errorResponse(res, 'Email đã được sử dụng. Vui lòng đăng nhập.', 400);
      }

      // Kiểm tra Google ID đã được sử dụng chưa
      const googleIdExists = await User.findByGoogleId(decoded.google_id);
      if (googleIdExists) {
        return errorResponse(res, 'Tài khoản Google này đã được đăng ký. Vui lòng đăng nhập.', 400);
      }
